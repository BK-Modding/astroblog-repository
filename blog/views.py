...
def star(request, post_id):
    if request.method == 'POST':
        ...
        if request.user.username != post.author.username:
            ...
            notify_author = Notification()
            notify_author.user_to_notify = post.author
            notify_author.content = '''<a href="/user/''' + str(request.user.id) + '''">''' + request.user.username + '''</a> starred the post "<a href="\posts\\''' + str(post_id) + '''">''' + post.title + '''</a>"''' ################ (This is done lol) USSSSSSSSSSSSERRRRRRRRRRRRRRRR PAGGGGGGGGGGGGGE FOR STARRING NOTIFY
            notify_author.save()
...
def keep(request, post_id):
    if request.method == 'POST':
        ...
        if request.user.username != post.author.username:
            notify_author = Notification()
            notify_author.user_to_notify = post.author
            notify_author.content = '''<a href="/user/''' + str(request.user.id) + '''">''' + request.user.username + '''</a> kept the post "<a href="\posts\\''' + str(post_id) + '''">''' + post.title + '''</a>"''' ################ (This is done lol) USSSSSSSSSSSSERRRRRRRRRRRRRRRR PAGGGGGGGGGGGGGE FOR STARRING NOTIFY
            notify_author.save()
            keep = Keep()
            keep.blog_post = post
